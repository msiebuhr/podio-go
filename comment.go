...
func (client *Client) Comment(refType string, refId float64, text string, params map[string]interface{}) (*Comment, error) {
	path := fmt.Sprintf("/comment/%s/%.0f/", refType, refId)
	if params == nil {
...
func (client *Client) GetComments(refType string, refId float64) (comments []*Comment, err error) {
	path := fmt.Sprintf("/comment/%s/%.0f/", refType, refId)
